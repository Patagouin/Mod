...
void Analysis::makeUnitary(std::vector<Eigen::Vector3f> &vertices)
{
    ...
    for(std::vector<Eigen::Vector3f>::iterator v_iter = vertices.begin() ; v_iter!=vertices.end() ; ++v_iter)
    {
    ...
    {
        vertices[i] = (vertices[i] - center) / m;
    }
}
